...
function bookGamesCount(b){ return b.soon?0:b.parts.reduce((s,p)=>s+p.games.length,0); }
...
function bookInner(b){
  return `<div class="book ${b.soon?'soon':''}" style="--c1:${b.c1};--c2:${b.c2}">
    ...
    <div class="face">
      ...
      <div class="imprint">${LANG==='ko'?'기술 생존 가이드':'Technology Survival Guide'}</div>
    </div>
  </div>`;
}
...
function renderShelf(){
  ...
  BOOKS.forEach(b=>{
    ...
    if(!b.soon){
      ...
      const style = d===0 ? 'style="color:var(--soft);background:#f1e8d3;border-color:var(--line);"' : '';
...
function renderDetail(id){
  ...
  back.textContent=UI.back[LANG]; back.setAttribute("href",ROOT);
  document.getElementById("foot2").textContent=UI.foot[LANG];
  document.title = T(b,'t')+" · "+UI.kicker[LANG];

  const hero=document.getElementById("hero"); hero.innerHTML="";
  const cover=document.createElement("div"); cover.innerHTML=bookInner(b); hero.appendChild(cover.firstElementChild);
